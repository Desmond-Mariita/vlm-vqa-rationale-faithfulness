...
The \emph{Sufficiency Gap} is this thesis's name for the accuracy gap under grey
replacement, not the ERASER sufficiency measure defined in
Section~\ref{sec:background-interventions}:
\[
...
processed visual tokens may still differ between arms. A smaller gap means that
accuracy is better preserved under this specific grey replacement; it does not
quantify how much answer-relevant information remains. In a description arm,
retained text may help preserve accuracy, so the gap is interpreted as
...
used \texttt{bert\_score==0.3.12} with \textsc{roberta-large}
\citep{DBLP:journals/corr/abs-1907-11692}, model snapshot
...
matches the answer supplied to Stage~2. This connects CMC to simulatability-style
explanation evaluation \citep{DBLP:conf/emnlp/HaseZXB20}, but CMC does not apply
their Leakage-Adjusted Simulatability correction for direct label leakage.
