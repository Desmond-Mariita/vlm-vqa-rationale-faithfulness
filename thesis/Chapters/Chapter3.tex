...
the evaluation subset, and records that no rule matched were set aside. For one
common model, overall validation accuracy was 63.52 versus 33.91 on the selected
counterexample subset; for a pretrained transformer, the corresponding accuracies
were 67.77 and 39.24. \citet{DBLP:conf/emnlp/SiMZLL0CWZ22} generalised the idea into nine
...
These studies do not test generated rationales under altered images.
...
These text interventions show that written reasoning can omit an influential cue
and that answer dependence on a written chain can be tested by editing it.
\citet{DBLP:conf/acl/AtanasovaCLLSA23} provide adjacent tests for natural-language
explanations: they insert reasons that change predictions without appearing in
the explanation, and reconstruct inputs from stated reasons to test whether
the prediction is preserved. \citet{ParcalabescuFrank24CCSHAP} argue that such
output tests measure self-consistency rather than faithfulness to hidden
computation; their CC-SHAP measure compares input contributions to answers and
explanations.

Visual and textual contributions have also been compared in VLM decoders.
\citet{ParcalabescuFrank25VLM} extend this framework to multimodal answer and
explanation generation, finding greater image contributions for explanations
than for answers in the models and tasks tested. This is a close antecedent to
the present study, whose focus is a matched VCR comparison under the same
image-intervention battery at both stages.
...
This thesis builds on these explanation tests and multimodal contribution
analyses by jointly evaluating Stage~1 answer behaviour and Stage~2 free-form rationale
...
separate and interpreting rationale Drift against empirical reference points.
...
Earlier tests also compare input contributions to answers and explanations.
This thesis pairs the same VCR records under one image-intervention battery at
both stages, with empirical Drift references \\
\bottomrule
\end{tabular}
\end{table}
